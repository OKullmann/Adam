...
\label{sec:prelimgraph}

\begin{defi}\label{def:graph}
  A \textbf{graph} is a pair $(V, E)$, where $V$ is the set of \textbf{vertices}, and $E$ is the set of \textbf{edges}, where an edge is a 2-element subset of $V$. Often a graph is denoted by $G = (V, E)$, and we may use $V(G) := V$ and $E(G) := E$.
\end{defi}

\begin{examp}\label{exp:exampgraph}
  some simple examples for graphs, using mathematical notation, and additionally drawing them XXX
  \begin{enumerate}
  \item The empty graph $(\emptyset,\emptyset)$.
  \item A graph with one vertex: $(\{1\}, \emptyset)$. Note that a graph with at most one vertex can not have an edge.
  \item There are exactly two possible graphs $G$ with $V(G) = \{1,2\}$, namely with $E(G) = \emptyset$ or $E(G) = \{\{1,2\}\}$.
  \end{enumerate}
\end{examp}


\section{Logic}
\label{sec:prelimlogic}
...
\begin{defi}\label{def:graphcolouring}
  Consider a graph $G$ (recall Definition \ref{def:graph}). A \textbf{colouring} of $G$ with colour-set $C$ is a map $f: V(G) \rightarrow C$ such that for each $\{u,v\} \in E(G)$ we have $f(u) \ne f(v)$. Such a colouring is called a \textbf{$k$-colouring} for some integer $k \ge 0$ if $\abs{C} \le k$. The default colour-set of a $k$-colouring is $\{1,\dots,k\}$.
\end{defi}
In other words, a grap colouring uses a colour-set $C$, and assigns to every vertex a ``colour'', that is, an element of $C$, such that adjacent vertices get different colours.

\begin{examp}\label{exp:graphcolour}
  XXX simple examples for graph colourings and graph-non-colourings XXX
\end{examp}
...
\begin{defi}\label{def:chromaticnumber}
  The \textbf{chromatic number} of a graph $G$, denoted by $\chi(G)$, is the smallest integer $k \ge 0$ such that $G$ has a $k$-colouring.
\end{defi}

\begin{examp}\label{exp:chi}
  The three most trivial examples:
  \begin{enumerate}
  \item $\chi((\emptyset,\emptyset)) = 0$; in general we have $\chi(G) = 0$ if and only $V(G) = \emptyset$.
  \item $\chi((\{1\},\emptyset)) = 1$; in general we have $\chi(G) = 1$ if and only if $E(G) = \emptyset$ and $V(G) \ne \emptyset$.
  \item 
  \end{enumerate}
 . XXX
\end{examp}
...
The following Lemma holds for the reduction from Latin square $\emph{L}$ to Sudoku $\emph{S}$. As when $\emph{i}$ = $\emph{j}$ = 3 : (0 * 3 + 1 + 3) mod 9 = 4 (which yields 11 in base 3). Also when $\emph{i}$ = 5 and $\emph{j}$ = 7 : (2 * 3 + 1 + 7) mod 9 = 5 (which yields 12 in base 3). The rest of the 9 x 9 can be filled out other than those grids which are $\in$ $\emph{B}$ ($\perp$).

\begin{center}
\includegraphics[scale=0.70]{Sud.png}
\end{center}

The blanks are those grids where $\emph{B}$ = $\lbrace$(i,j) $\vert$ $\lfloor$$\frac{i}{n}$$\rfloor$ = 0 and (j mod n) = 0$\rbrace$. This holds for all 9 of the blank entries as when i = j = 0 : $\lfloor$0/3$\rfloor$ = 0 and (0 mod 3). The other numbers where the constraints for blanks hold would be 3 and 6.

\begin{center}
\includegraphics[scale=0.80]{Sud2.png}
\end{center}

The figure above shows a Latin square $\emph{L}$ which has been filled out such that each row and column contains each of the integers from 0 through to $\emph{n}$ - 1 exactly once. I would now need to create a Latin square $\emph{L}$ with the following blank number 0,3 and 6 which would look as follows;

\begin{center}
\includegraphics[scale=0.85]{Sud3.png}
\end{center}

This would then be blown up into the 9 x 9 Sudoku. As the first three blanks in the left column of the Sudoku will be the entries from the left column of the Latin square. Then the second three blanks of the Sudoku will be filled with the entries from the second column of the Latin. The next step would be the third three blanks of the Sudoku will be filled with the entries from the third column of the Latin square. The final process blowing up of the Sudoku would be convert the numbers from Binary to Tenary.

The blown up Sudoku above shows that every Sudoku constraints hold such that every row, column and sub-grid contains integers from 0 through to n - 1 exactly once. 

\begin{center}
\includegraphics[scale=0.85]{Sud4.png}
\end{center}

\section{Latin Square}
\label{sec:LatinSquare}

\begin{defi}\label{def:latinsquare}
A Latin square is a n x n grid in which some of the boxes are filled with numbers from 1 to n, there are also blanks such as the Sudoku that need to be filled, such that every row and column contains each of the n possible number. [6]. This is very much similar to Sudoku but the only difference is that Latin square don't have the small grid constraint. An example of Latin square looks as follows;
\end{defi}

\begin{center}
\includegraphics[scale=0.85]{Sud5.png}
\end{center}

Lemma for a given partial Latin Square $\emph{L}$ or of order n we construct a Sudoku $\emph{S}$ as follows;
